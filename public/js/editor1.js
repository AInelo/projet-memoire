...
publishBtn.addEventListener("click", () => {
  if (articleFeild.value.length && blogTitleField.value.length) {
    ...
    db.runTransaction((transaction) => {
      ...
      return transaction
        .get(db.collection("lastIdCollection").doc("lastIdDoc"))
        .then((doc) => {
          ...
          if (doc.exists) {
            ...
            return db
              .collection("Bloglasmo")
              .doc(nextId.toString())
              .set(newBlog)
              .then(() => {
                // Mettre à jour la valeur de l'ID dans le document "lastIdDoc"
                return transaction.update(
                  db.collection("lastIdCollection").doc("lastIdDoc"),
                  {
                    currentId: nextId,
                  }
                );
...
const addImage2 = (imagepath, alt) => {
  ...
  let textToInsert = `\r![${alt}](${imagepath})\r`;
  OuvrageAbstract.value =
    OuvrageAbstract.value.slice(0, curPos) +
    textToInsert +
    OuvrageAbstract.value.slice(curPos);
};
...
db.collection("InscritFormation")
  .get()
  .then((messages) => {
    messages.forEach((MAILS) => {
      if (MAILS.id != decodeURI(location.pathname.split("/").pop())) {
...
const addImage3 = (imagepath, alt) => {
  ...
  let textToInsert = `\r![${alt}](${imagepath})\r`;
  OuvrageAbstractEvent.value =
    OuvrageAbstractEvent.value.slice(0, curPos) +
    textToInsert +
    OuvrageAbstractEvent.value.slice(curPos);
};
